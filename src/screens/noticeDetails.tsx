...
export default function NoticeDetails({route}: any) {
  const {id, image} = route.params?.NoticeDetails;
  const [noticeDetails, setNoticeDetails] = React.useState({} as any);
  const [loading, setLoading] = React.useState(true);
  React.useEffect(() => {
    async function fetchData() {
      try {
        const response = await fetch(
          `https://jobhunter.btebresultsbd.com/noticeDetails/${id}`,
        );
        const json = await response.json();
        setNoticeDetails(json?.notice);
        setLoading(false);
      } catch (error) {
        showMessage({
          ...
          <>
            <View style={styles.mainBox}>
              ...
              <Text style={styles.source}>
                <Image source={Source} style={{width: 20, height: 20}} /> উৎস:{' '}
                {noticeDetails?.source}
              </Text>
            </View>
            ...
            <View style={styles.detailsview}>
              <RenderHtml
                contentWidth={width}
                source={{html: noticeDetails?.content}}
                baseStyle={{
                  fontSize: 16,
                  color: colors.white,
                  marginBottom: 10,
